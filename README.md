...
- **Installing into a pre-existing namespace clobbers remnants** — When the installer owns the namespace lifecycle (creates on install, deletes on `uninstall_delete_all`), a target namespace that already exists holds another workload or leftovers from a prior failed install (OperatorGroup, operator Subscription/CSV, orphaned secrets). Proceeding causes Helm "invalid ownership metadata" errors and overwrites what's there. Detect the existing namespace and abort: enforce it as a `prerequisites_failed` in `check_pre_reqs.sh` (INSTALL runs prereqs first, so it fails fast with no partial state) plus a defense-in-depth guard in `install.sh`. Give an actionable message (delete the namespace or run `uninstall_delete_all`, or pick an unused namespace). Skip this only for quickstarts designed to deploy into a shared/pre-provisioned namespace.
