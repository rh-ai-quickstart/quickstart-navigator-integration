...
- Required CRDs
- Target namespace availability — if the installer owns the namespace lifecycle, treat an *existing* target namespace as a prerequisite failure (see the namespace collision policy under `install.sh`). This makes INSTALL abort fast, and makes a standalone `check_pre_reqs` run flag the collision before the user attempts an install.
...
**`install.sh`** — Must:
- Create the target namespace — and decide the collision policy up front (see below)
- Invoke the existing deployment mechanism (helm install, make install, apply manifests, etc.)

**Namespace collision policy — refuse a pre-existing namespace when the installer owns the namespace lifecycle:** Most single-quickstart installers own a *dedicated* target namespace: they create it on install and delete it on `uninstall_delete_all`. For these, a target namespace that *already exists* at install time was NOT created by this run — it holds either another workload or remnants of a previous (often failed) install: a leftover OperatorGroup, an operator Subscription/CSV, orphaned secrets/PVCs. Installing into it silently is dangerous: Helm hits "invalid ownership metadata" errors on resources it didn't create, and you clobber whatever was there. The safe default is to **detect the existing namespace and abort** rather than proceed.
  - Enforce it as a **prerequisite failure in `check_pre_reqs.sh`** (add to the `missing` array) so INSTALL — which runs `check_prerequisites` first — fails fast *before* installing operators or running Helm, leaving no partial state. Emit a `prerequisites_failed` (exit 2) with an actionable message: delete the namespace (`oc delete namespace <ns>`) or run `uninstall_delete_all`, or pick a namespace that doesn't yet exist.
  - Add a **defense-in-depth guard in `install.sh`** too (`if oc get namespace "$TARGET_NAMESPACE" >/dev/null 2>&1; then log_error ...`), for the case where the deploy function is invoked directly or the namespace appears between the prereq check and namespace creation (TOCTOU). After the guard, create the namespace unconditionally (`oc create namespace`) — don't fall back to "already exists, proceed".
  - Confirm which side creates the namespace: the client-side `deploy.sh` must NOT pre-create the target namespace, or the guard would trip on every run. In the standard pattern the installer (container-side) owns creation.
  - **Exception:** if the quickstart is *designed* to deploy into a shared or pre-provisioned namespace (e.g. `redhat-ods-applications`), skip this guard — collision is expected there. Ask the engineer which model applies before choosing.
